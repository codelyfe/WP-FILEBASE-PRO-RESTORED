...
// returns true if range download should be supported for the specified file/file type
    ...
    )
    {
        ...
        if (empty($url) || !parse_url($url, PHP_URL_SCHEME)) return array('error' => 'No URL for sideload given!');
        ...
        if ($rh === false) {
            return array(
                'error' => sprintf('Could not open URL %s (%s)!', $url, $http_response_header[0]) . ' '
        ...
        if (!is_dir($dest_dir)) {
            ...
            $res = WPFB_Admin::Mkdir($dest_dir);
            if ($res['error'])
                return array('error' => "Failed to create dir $dest_dir!");
        }
        ...
        if ($fh === false) {
            ...
            return array(
                'error' => sprintf('Could not create file %s! (%s)', $dest_path, json_encode($e))
            );
        }
        ...
        while (!feof($rh)) {
            ...
            if (is_callable($progress_bar_or_callback)) {
                call_user_func($progress_bar_or_callback, $size, $total_size);
            } elseif (is_object($progress_bar_or_callback) && method_exists($progress_bar_or_callback, 'set')) {
    ...
    static function DownloadCategoryZip($cat)
    {
        ...
        if (empty($files))
            wp_die("No files in $cat!");

        // build a hash
        $hash_string = '';
        foreach ($files as $file) {
            $hash_string .= $file->file_path . $file->file_size
                . $file->file_mtime . $file->file_hash;
        }
        $hash = hash('md4', $hash_string);
        unset($hash_string);

        $dir = WPFB_Core::UploadDir() . '/.tmp/catzip';
        ...
        if (!file_exists($tmp_zip_file_path)) {
            ...
            $tmp_files = array();
            $zip_dirs = array();
            $path_cut_len = strlen(trim(str_replace('\\', '/',
                dirname($cat->GetLocalPathRel())), '/.'));
            foreach ($files as $file) {
                $entry_path = trim(substr($file->GetLocalPathRel(),
                    $path_cut_len), '/\\');
                if ($file->IsRemote()) {
                    $res = wpfb_call('Admin', 'SideloadFile', $file);
                    if (!empty($res['error'])) {
                        wp_die($res['error']);
                    }
            ...
            touch($tmp_zip_file_path);
        }

        // delete catzips after 30 minutes
        wpfb_loadclass('FileUtils');
        WPFB_FileUtils::DeleteOldFiles($dir, 60 * 30);

        do_action('wpfilebase_catzip_downloaded', $cat->cat_id);

        self::SendFile($tmp_zip_file_path,
            array('filename' => sanitize_file_name($cat->GetTitle()) . ".zip"));
