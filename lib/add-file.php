...
<?php } ?>
		...
		-->
		<tr class="form-field">
			<th scope="row" valign="top"><label for="file_sec_cat1"><?php _e('Secondary Categories', 'wp-filebase') ?></label></th>
			<td colspan="3">
	...
	<?php } else { ?><th></th><td></td><?php } ?>
				...
				<td>
					<fieldset><legend class="hidden"><?php _e('Direct linking') ?></legend>
						...
						<br /><label title="<?php _e('No') ?>"><input type="radio" name="file_direct_linking" value="2" <?php checked('2', $file->file_direct_linking); ?>/> <?php _e('Redirect to File Page', 'wp-filebase') ?>
							<?php
							if ($update) {
...
<?php if ($exform) { ?>
	...
	<?php _e('Limit file access by selecting one or more user roles.') ?>
		...
		?>
		<tr class="form-field">
			<th scope="row" valign="top"><label for="file_password"><?php _e('File Password', 'wp-filebase') ?></label></th>
